...
for i in urls:
    try:
        ...
        photo3_ = base_url + soup.find('div', class_='slider-nav2-start').find_next('img').find_next('img').find_next('img')['src']
        ...
        try:
           args.append((id, name_, price_,photo1_,photo2_,photo3_,description_,Version_OS_, corpus_, sim_card_, size_, weight_, display_type_, diagonale_, allow_display_,photo_kamera_, front_kamera_, audio_, standart_,Interface_, prochessor_,yadra_, giga_vstoeno_, giga_operate_,sloy_card_, accumulator_, dop_infa_,slug_,brand_,audio1_,types_,count_,rating_))
        except:
            id -= 1
            count += 1
        else:
...
cur = con.cursor()
execute_values(cur,"INSERT INTO phones_phones (id, name, price,photo1,photo2,photo3,description,version_os, corpus, sim_card, size, weight, display_type, diagonale, allow_display,photo_kamera, front_kamera, headphone_jack, standart,interface, prochessor,yadra, giga_vstoeno, giga_operate,sloy_card, accumulator, dop_infa,slug,brand,audio,types,count,rating) VALUES %s",args)
con.commit()
con.close()