...
class AirPods_comments_APP(APIView):
    ...
    def h1(self, kw, request):
        pk = kw.get('pk', None)  # pk работает как айди объекта
        if request.method == "GET":
            self.instance = AirPods_comments.objects.all().filter(airpod_id__in=[pk]).select_related('accounts',
                                                                                                     'airpod')
            if len(self.instance) == 0:
                raise serializers.ValidationError({1: 'ok'})
        elif request.method == "PUT" or request.method == "DELETE":
...
class Bascet_products_APP(APIView):
    ...
    def h1(self, kw, request):
        ...
        if request.method == "GET":
            if user.id == pk:
                self.instance = Bascet_products.objects.filter(accounts_id__in=[pk]) & Bascet_products.objects.filter(
                    product_buy=False)
            else:
                raise serializers.ValidationError({1: 'ok'})
            if len(self.instance) == 0:
    ...
    def patch(self, request, *args, **kwargs):
        ...
        id=63
        for i in a:
            z= Bascet_products.objects.filter(accounts_id__in=[id]) & Bascet_products.objects.filter(product_buy=True) & Bascet_products.objects.filter(time=i)
            b= self.instance & Bascet_products.objects.filter(time=i)
            print(b)
            print(z)
        return Response(Buscet_products_Seria(self.instance, many=True).data)
